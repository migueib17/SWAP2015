...
*Al ejecutar este comando tuve un problema con el ssh: Òconection refused port 22Ó.
SolucionŽ el problema f‡cilmente actualizando ssh en las dos m‡quinas: sudo apt-get install ssh y puse las dos m‡quinas virtuales de vmware en puente.
...
2. Instalar la herramienta rsync
	...
	1. sudo su
	2. contrase–a
	3. apt-get install rsync 	Ñ> (ya lo ten’a actualizado)
	
	En m‡quina2:
	4. rsync -avz -e ssh root@192.168.1.50:/var/www/ /var/www/
	(clona la carpeta de la m‡quina1 en nuestra m‡quina).

*Al ejecutar este comando tuve un problema: "protocol version mismatch -- is your shell clean?Ó. Este problema se debe a que tengo el prompt modificado, entonces tuve que dejar el archivo .bashrc como ven’a de f‡brica, para que ssh no me diera problemas.
